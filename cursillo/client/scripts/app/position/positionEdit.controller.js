angular.module('app').controller('PositionEditController', ['$scope', 'Position', '$stateParams', function ($scope, Position, $stateParams) {
  ...
  $scope.createPosition = function () {
    Position.create($scope.position, function (res) {
      $state.go('position.detail', res.id);
    });
  ...
  $scope.updatePosition = function () {
    Position.update({id: $scope.positionId}, $scope.position, function (position) {
      $state.go('position.detail', position.id);
    });
  };
